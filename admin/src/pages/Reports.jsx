...
export default function Reports() {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="panel p-4">
        <div className="flex flex-col lg:flex-row lg:items-end gap-4">
          ...
          <div className="flex-1">
            <p className="label mb-2">Quick Range</p>
            <div className="flex flex-wrap gap-2">
              {PRESETS.map((p) => (
                <button
                  key={p.label}
                  onClick={() => applyPreset(p)}
                  className={`border-2 px-3 py-1.5 text-[11px] font-bold uppercase tracking-wider transition-all ${
      ...
      {error && (
        <div className="panel p-4 flex items-center gap-2.5 bg-red-50 border-2 border-red-600 text-red-800 text-xs font-semibold">
      ...
      {loading ? (
        ...
        <div id="report-print-area" className="space-y-5">
          {/* Masthead (prints) */}
          <div className="bg-ink border-2 border-ink px-6 py-5 flex items-center justify-between print:shadow-none">
            <div className="flex items-center gap-3">
              <div className="w-11 h-11 bg-flame-500 border-2 border-paper-100 flex items-center justify-center">
                <ChartPieSlice className="w-5 h-5 text-ink" weight="fill" />
              </div>
              <div>
                <h4 className="display text-xl text-white leading-none tracking-wide">REVENUE REPORT</h4>
          ...
          {/* Summary */}
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
            <div className="panel p-5 border-2 border-ink shadow-hard-flame">
              <p className="label">Total Revenue</p>
              <h3 className="display text-2xl text-ink mt-1.5 tabular-nums">{inr(summary.totalRevenue)}</h3>
              <p className="text-[11px] text-ink-mute font-medium mt-1.5 flex items-center gap-1">
                <TrendUp className="w-3.5 h-3.5 text-flame-600" weight="bold" /> {rangeText(from, to)}
              </p>
            </div>
            <div className="panel panel-hover p-5">
              <p className="label">Unlocks</p>
              <h3 className="display text-2xl text-ink mt-1.5 tabular-nums">{summary.totalUnlocks || 0}</h3>
              <p className="text-[11px] text-ink-mute font-medium mt-1.5">successful purchases</p>
            </div>
            <div className="panel panel-hover p-5">
              <p className="label">Months Covered</p>
              <h3 className="display text-2xl text-ink mt-1.5 tabular-nums">{summary.monthCount || 0}</h3>
              <p className="text-[11px] text-ink-mute font-medium mt-1.5">with activity</p>
            </div>
            <div className="panel panel-hover p-5">
              <p className="label">Top Template</p>
              <h3 className="display text-xl text-ink mt-2 truncate" title={byTemplate[0]?.template?.name}>
                {byTemplate[0]?.template?.name || '—'}
              </h3>
              <p className="text-[11px] text-ink-mute font-medium mt-1.5">
          ...
          <div className="panel p-6">
            <div className="flex items-center justify-between mb-5">
              <h3 className="display font-bold text-ink text-sm flex items-center gap-2">
                <Receipt className="w-5 h-5 text-flame-600" weight="duotone" /> Monthly Revenue
              </h3>
              <span className="label">By calendar month</span>
            </div>
          ...
          <div className="panel overflow-hidden">
            <div className="px-6 pt-6 pb-4 flex items-center justify-between">
              <h3 className="display font-bold text-ink text-sm flex items-center gap-2">
                <CrownSimple className="w-5 h-5 text-glow-600" weight="duotone" /> Revenue by Template
              </h3>
